...
export class FlowController {
  ...
  static async getFlows(req: Request, res: Response): Promise<void> {
    try {
      const flows = await ChatbotFlowModel.findAll();
  ...
  static async getFlowById(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const flow = await ChatbotFlowModel.findById(id);
  ...
  static async createFlow(req: Request, res: Response): Promise<void> {
    try {
      const { name, description, flow_definition }: CreateFlowRequest = req.body;
      ...
      const existingFlow = await ChatbotFlowModel.findByName(name.trim());
      if (existingFlow) {
        res.status(400).json({
      ...
      const flowData = {
        name: ValidationUtils.sanitizeInput(name.trim()),
        description: description ? ValidationUtils.sanitizeInput(description.trim()) : undefined,
        flow_definition: flow_definition || {},
        is_active: false // Novo fluxo sempre inicia inativo
      };

      const flow = await ChatbotFlowModel.create(flowData);
  ...
  static async updateFlow(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { name, description, flow_definition, is_active }: UpdateFlowRequest = req.body;
      ...
      const existingFlow = await ChatbotFlowModel.findById(id);
      if (!existingFlow) {
        res.status(404).json({
          error: 'Fluxo não encontrado'
        });
      ...
      if (name !== undefined) {
        ...
        const duplicateFlow = await ChatbotFlowModel.findByName(name.trim());
        ...
        updateData.name = ValidationUtils.sanitizeInput(name.trim());
      }

      if (description !== undefined) {
        updateData.description = description ? ValidationUtils.sanitizeInput(description.trim()) : null;
      }
      ...
      const updatedFlow = await ChatbotFlowModel.update(id, updateData);
  ...
  static async deleteFlow(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const deleted = await ChatbotFlowModel.delete(id);
  ...
  static async publishFlow(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      await ChatbotFlowModel.deactivateAll();

      // Ativar o fluxo atual
      const updatedFlow = await ChatbotFlowModel.update(id, { is_active: true });
  ...
  static async unpublishFlow(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      // Desativar o fluxo
      const updatedFlow = await ChatbotFlowModel.update(id, { is_active: false });
  ...
  static async getActiveFlow(req: Request, res: Response): Promise<void> {
    try {
  ...
  static async getFlowStats(req: Request, res: Response): Promise<void> {
    try {
      const flows = await ChatbotFlowModel.findAll();
