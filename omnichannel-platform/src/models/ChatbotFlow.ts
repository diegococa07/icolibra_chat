...
export class ChatbotFlowModel {
  
  // Criar um novo fluxo de chatbot
  static async create(flowData: CreateChatbotFlow): Promise<ChatbotFlow> {
    const id = uuidv4();
  ...
  // Buscar fluxo por ID
  static async findById(id: string): Promise<ChatbotFlow | null> {
  ...
  static async findByName(name: string): Promise<ChatbotFlow | null> {
  ...
  static async findAll(): Promise<ChatbotFlow[]> {
  ...
  static async findAllActive(): Promise<ChatbotFlow[]> {
  ...
  static async findMainActive(): Promise<ChatbotFlow | null> {
  ...
  static async findActive(): Promise<ChatbotFlow | null> {
    return await this.findMainActive();
  }

  // Atualizar fluxo
  static async update(id: string, updateData: Partial<CreateChatbotFlow>): Promise<ChatbotFlow | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;
  ...
  static async activate(id: string): Promise<ChatbotFlow | null> {
    // Primeiro, desativar todos os fluxos
    await query('UPDATE chatbot_flows SET is_active = false');
  ...
  // Desativar fluxo
  static async deactivate(id: string): Promise<ChatbotFlow | null> {
    const sql = `
      UPDATE chatbot_flows 
      SET is_active = false
      WHERE id = $1
  ...
  static async deactivateAll(): Promise<number> {
  ...
  // Duplicar fluxo
  static async duplicate(id: string, newName: string): Promise<ChatbotFlow | null> {
  ...
  static validateFlowDefinition(flowDefinition: any): {
    ...
    const hasStartNode = flowDefinition.nodes?.some((node: any) => node.type === 'start');
    if (!hasStartNode) {
    ...
    if (flowDefinition.nodes) {
      flowDefinition.nodes.forEach((node: any, index: number) => {
        if (!node.id) {
  ...
  static async findByDateRange(startDate: Date, endDate: Date): Promise<ChatbotFlow[]> {
    const sql = `
  ...
  static async exportFlow(id: string): Promise<{
    metadata: {
      id: string;
      name: string;
      exported_at: Date;
  ...
  static async importFlow(importData: {
    metadata: any;
    flow: any;
