...
exports.SystemMessageController = void 0;
...
class SystemMessageController {
    ...
    static async list(req, res) {
        try {
            const messages = await models_1.SystemMessageModel.findAll();
            res.json({
                success: true,
                data: messages,
    ...
    static async getByKey(req, res) {
        try {
            ...
            const message = await models_1.SystemMessageModel.findByKey(key);
            if (!message) {
                res.status(404).json({
                    success: false,
    ...
    static async updateByKey(req, res) {
        try {
            ...
            const existingMessage = await models_1.SystemMessageModel.findByKey(key);
            if (!existingMessage) {
                res.status(404).json({
                    success: false,
                    error: 'Mensagem não encontrada'
            ...
            const updatedMessage = await models_1.SystemMessageModel.updateByKey(key, content.trim());
            res.json({
                success: true,
                data: updatedMessage,
    ...
    static async create(req, res) {
        try {
            ...
            const existingMessage = await models_1.SystemMessageModel.findByKey(message_key.trim().toUpperCase());
            if (existingMessage) {
                res.status(409).json({
                    success: false,
            ...
            const newMessage = await models_1.SystemMessageModel.create(message_key.trim().toUpperCase(), content.trim(), description?.trim());
            res.status(201).json({
                success: true,
                data: newMessage,
    ...
    static async deleteByKey(req, res) {
        try {
            ...
            await models_1.SystemMessageModel.delete(key);
            res.json({
                success: true,
    ...
    static async getContentByKey(req, res) {
        try {
            ...
            const content = await models_1.SystemMessageModel.getMessageContent(key);
            if (!content) {
                res.status(404).json({
                    success: false,
                    error: 'Mensagem não encontrada'
...
exports.SystemMessageController = SystemMessageController;
//# sourceMappingURL=SystemMessageController.js.map